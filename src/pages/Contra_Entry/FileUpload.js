...
function FileUpload() {
  ...
  const btn_auct_upld_click = async e => {
    ...
    try {
      const res = await axios.post('http://localhost:3001/auct_price_upload', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });

      const res_auct = await axios.post('http://localhost:3001/process-auct-price');
  ...
  const btn_oblg_upld_click = async e => {
    ...
    try {
      const res = await axios.post('http://localhost:3001/oblig_file_upload', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
  ...
  const btn_Obligation_Click = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:3001/process-obligation');
