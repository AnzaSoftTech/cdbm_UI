...
function Client_Links_Popup({ onCloseClick, p_cli_id, p_cli_name }) {
    ...
    const handleGetCliLinks = async () => {
        try {
            const response = await axios.get('http://localhost:3001/api/get_linked_client', {
                params: {
                    p_client_code: p_cli_id
                }
            });
    ...
    const handleFinalSave = () => {
        ...
        axios.post('http://localhost:3001/api/upd_client_link', data)
            .then(response => {
    ...
    const handleDelete = (code) => {
        ...
        axios.post('http://localhost:3001/api/delete_client_link', data)
            .then(response => {
    ...
    const handleDeleteAll = () => {
        ...
        axios.post('http://localhost:3001/api/delete_client_links', data)
            .then(response => {
    ...
    const fetchfunc = async (code) => {
        try {
            const response = await axios.get('http://localhost:3001/api/get_client_name', {
                params: {
                    p_client_cd: code,
                }
            });
                ...
                <div className="card-body">
                    ...
                    <div className="row">
                        ...
                        <div className="col-md-6 mb-2 d-flex">
                            <label htmlFor="CliGrpName" className="form-label label-width">Client Group</label>
                            <input id="CliGrpName" readOnly type="text" className="form-control size_input_cashbank"
                                value={p_cli_name} />
                        </div>
                    </div>
                    <div className="row">
                            ...
                            <Modal show={showModalCliName} onHide={() => setShowModalCliName(false)}>
                                ...
                                <Modal.Body>
                                    <PopupSearchClientName  onRowSelect={handleCliRowSelect} 
                                     p_client_id={p_cli_id} getClients={handleGetCliLinks} close={setShowModalCliName}/>
                                </Modal.Body>
                                <Modal.Footer>
                        ...
                        <tbody>
                            {cliNameResults.map((result, index) => (
                                <tr key={index} style={{ cursor: 'pointer' }}>
                                    ...
                                    <td><button className='btn btn-secondary' onClick={()=>handleDelete(result.cli_cd?result.cli_cd:result.client_cd)}>Delete</button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
