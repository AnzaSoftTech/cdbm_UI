...
const PopupSearchClientName = ({ onRowSelect, onCheckBoxRowSelect, p_family_id, getClients, close }) => {
    ...
    const handleSaveAll = () => {
        ...
        if (cliNameResults.length === 0) {
            alert('Please select Client(s) to add to Family Group.');
            return;
        }
        ...
        axios.post(`${BASE_URL}/api/upd_client_links`, data)
            .then(response => {
